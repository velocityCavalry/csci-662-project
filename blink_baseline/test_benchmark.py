...
import argparse
from query_model import query_gpt4v, query_llava_7b
        ...
        for split in ['val']:
            ...
            for orig_d in tqdm(test_data):
                ...
                outputs[split].append(
                    {'idx': idx, 'answer': gold_answer, 'full_prediction': gpt_answer, 'prediction': prediction})
...
def parse_args():
    ...
    parser.add_argument("--model_name", type=str, default='GPT4V', help="select the model name",
                        choices=['GPT4V', 'llava7b'])
    parser.add_argument("--task_name", type=str, default='Relative_Depth', help="select the task name")
    parser.add_argument("--output_save_folder", type=str, default='outputs', help="select the output save folder")
