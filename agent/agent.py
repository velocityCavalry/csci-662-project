...
class SketchpadUserAgent(MultimodalUserProxyAgent):
    ...
    def sender_hits_max_reply(self, sender: Agent):
        return (
            self._consecutive_auto_reply_counter[sender.name]
    ...
    def receive(
        ...
        if not parsed_status:
            ...
            reply = self.prompt_generator.get_parsing_feedback(
                parsed_error_message, parsed_error_code
            )
    ...
    def initiate_chat(
        self, assistant, message, n_image, task_id, log_prompt_only=False
    ):
